		 ...
		 * 并行计算所有的预测解向量的优先级
		 * */
		__global__ void calaPriority(PriorityDouble* dev_priority,CoodinateDouble* dev_calaArray,
				const int row,const int Size)
		{
			int i = threadIdx.x + blockIdx.x*blockDim.x;
			if( i < Size )
			{
				double pri = 0.0;
				CoodinateDouble* a1 = dev_calaArray + i + 0 * Size;
				CoodinateDouble* a2 = dev_calaArray + i + 1 * Size;
				CoodinateDouble* a3 = dev_calaArray + i + 2 * Size;
				...
				//下面是测试代码
				bool isOne = (a1->x == a2->x) && (a2->x == a3->x);
				bool isCovered = a1->isCovered && a2->isCovered && a3->isCovered;
				bool isValid= a1->isValid && a2->isValid && a3->isValid;
				if(isCovered == true)
				{
					printf("Cala Prioruty Wrong,index:%d: (%f,%f,%d,%d) ,(%f,%f,%d,%d) ,(%f,%f,%d,%d) , isOne:%d, isCovered:%d  ,isValid:%d  \n",
							i,
							a1->x,a1->y,a1->isCovered,a1->isValid,
							a2->x,a2->y,a2->isCovered,a2->isValid,
							a3->x,a3->y,a3->isCovered,a3->isValid,
							isOne,isCovered,isValid);
				}
