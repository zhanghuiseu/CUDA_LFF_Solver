	...
	{
		SolverParameter::MAX_NUM_OF_PREDICT_PARAM = 80;
	 ...
	 * */
	void ATG::adjustParameter(const int level)
	{
		int para = SolverParameter::MAX_NUM_OF_PREDICT_PARAM;
		if(para <= SolverParameter::MinParameter)
			return ;
		else
		{
			//这里可以按照片一个恒定的衰减率取减少para，也可以随机衰减
			double k = PCATG::random();
			double kk = 0.03;
			para = para*(1-level*kk);
			SolverParameter::MAX_NUM_OF_PREDICT_PARAM = para;
		}
	}
