...
public:
	 ...
	 * 这个是CUDA并行化的入口
	 * */
	static bool generateTestDataForSolver(const int MaxTryParamSize);
	 ...
	 * 依据的是区间细化规则和扩展规则
	 * */
	static PredictValue* collectValidZero(vector<int> num,PredictValue* dev_ZerosValue,const int calaArraySize,const int row,
