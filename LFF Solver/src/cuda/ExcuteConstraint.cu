	 ...
	 * 第0个约束的CUDA计算函数
	 * */ 
	__device__ void getRuntimeValue_0_0(FloatType a , FloatType b , Coodinate* res )
	{
		res->y = ( powf((1-a),2.0)+100*(powf((b-a*a),2)) ) - ( 0 );
	...
	__global__ void calaFinalIntervel(Interval* dev_finalIntervel,Interval* dev_interval,const int calaArraySize)
	{
		...
		if(condition)
		{
			...
			bool hasIntervel = a0->hasIntervel;
	...
	__global__ void calaPriority(Priority* dev_priority,Coodinate* dev_calaArray,const int row,const int Size)
	{
		...
		if( i < Size )
		{
			...
			//下面是测试代码
			bool isOne = true;
			bool isCovered = a0->isCovered;
			bool isValid= a0->isValid;
			if(isCovered == true)
			{
				printf("Cala Prioruty Wrong,index:%d: (%f,%f,%d,%d) , isOne:%d , isCovered:%d , isValid:%d  \n",i,a0->x,a0->y,a0->isCovered,a0->isValid,isOne,isCovered,isValid);
			}
		}
	}
